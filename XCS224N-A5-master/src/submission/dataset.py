...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # - IMPORTANT: You are free to decide how to perform this random truncation, 
        # but make sure that the length is picked _randomly_ 
        # (every possible length from 4 to int(self.block_size*7/8) has a chance of being picked) for full credit.
        document = document[:int( torch.randint(low=4, high=int(self.block_size*7/8) + 1, size=(1,))[0] )]
        ...
        # The length of [masked_content] should be random, 
        # and 1/4 the length of the truncated document on average.
        ...
        len_ = int( torch.randint( low=1, high=2*int(len(document)/4), size=(1,))[0])
        idx_ = int( torch.randint( low=0, high=len(document) - int(len(document)/4) + 1, size=(1,))[0])
        
        prefix         = document[           : idx_]
        masked_content = document[idx_       : idx_ + len_]
        suffix         = document[idx_ + len_:]
        ...
        pad_string = self.PAD_CHAR * (self.block_size-len(document)-1)

        # Intuitively, the [masked_content], a string, is removed from the document and replaced with MASK_CHAR (the masking character defined in Vocabulary Specification). 
        # After the suffix of the string, the MASK_CHAR is seen again, followed by the content that was removed, and the padding characters.
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + pad_string
        ...
        # 5. Making use of the vocabulary that you defined, 
