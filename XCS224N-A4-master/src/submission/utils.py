...
def pad_sents(sents, pad_token):
    ...
    @param sents (list[list[str]]): list of sentences, where each sentence is represented as a list of words
    @param pad_token (str): padding token
    @returns sents_padded (list[list[str]]): 
    ...
    # length_list[i] stores the lengh of sentence[i]
    length_list = np.array( [ len(sentence) for sentence in sents ] )
    max_length = np.max(length_list)
    sents_padded = [ sents[i] + [pad_token] * (max_length-length) for i, length in enumerate( length_list ) ]
    ### END CODE HERE
