...
def train_for_epoch(parser, train_data, dev_data, optimizer, loss_func, batch_size, device):
    ...
    with tqdm(total=(n_minibatches)) as prog:
        for i, (train_x, train_y) in enumerate(minibatches(train_data, batch_size)):
            ...
            ###         This will take `logits` and `train_y` as inputs.
            ###         It will output the CrossEntropyLoss btw softmax(`logits`) and `train_y`.
            ###         Remember that softmax(`logits`) are the predictions (y^ from the PDF).
            ###      3) Backprop losses
