...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        ###        initializing the weight matrix with the `nn.init.xavier_uniform_` function with `gain = 1` (default)
        ...
        ### Hints:
        ###     - After you create a linear layer you can access the weight matrix via:linear_layer.weight
        ###
        ### Please see the following docs for support:
        ###     1. Linear Layer: https://pytorch.org/docs/stable/nn.html#torch.nn.Linear
        ###     2. Xavier Init: https://pytorch.org/docs/stable/nn.html#torch.nn.init.xavier_uniform_
        ###     3. Dropout: https://pytorch.org/docs/stable/nn.html#torch.nn.Dropout
        
        ### START CODE HERE (~5 Lines)
        # 1) Construct `self.embed_to_hidden` linear layer
        in_features_ = self.embed_size * self.n_features #size of each input sample
        out_features_ = out_features_ = self.hidden_size #size of each output sample
        self.embed_to_hidden = nn.Linear( in_features_, out_features_ )
        # initializing the weight matrix with the `nn.init.xavier_uniform_` function with `gain = 1` (default)
    ...
    def embedding_lookup(self, t):
        ...
        ###
        ###  Please see the following docs for support:
        ###     1. Embedding Layer: https://pytorch.org/docs/stable/nn.html#torch.nn.Embedding
        ###     2. View: https://pytorch.org/docs/stable/tensors.html#torch.Tensor.view
    ...
    def forward(self, t):
        ...
        # 3) Apply relu non-linearity to the output of step 2 to get the hidden units.
        # 4) Apply dropout layer to the output of step 3.
        hidden = self.dropout( nn.functional.relu( self.embed_to_hidden(embedding) ) )
        # 5) Apply `hidden_to_logits` layer to the output of step 4 to get the logits.
